...
void example_joiner::persist_metrics(VW::metric_sink& metrics)
{
  if (_vw->example_parser->metrics)
  {
    ...
    if (!_joiner_metrics.first_event_id.empty())
    {
      ...
      metrics.set_string("first_event_time", std::move(date::format("%FT%TZ", date::floor<std::chrono::microseconds>(_joiner_metrics.first_event_timestamp))), true);
    }
    if (!_joiner_metrics.last_event_id.empty())
    {
      metrics.set_string("last_event_id", std::move(_joiner_metrics.last_event_id), true);
      metrics.set_string("last_event_time", std::move(date::format("%FT%TZ", date::floor<std::chrono::microseconds>(_joiner_metrics.last_event_timestamp))), true);
    }
  }
}
