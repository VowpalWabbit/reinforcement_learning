...
gmt_now_and_timestamp()
{
  ...
  return std::make_pair(reinforcement_learning::messages::flatbuff::v2::TimeStamp(int(ymd.year()),
                            unsigned(ymd.month()), unsigned(ymd.day()), time.hours().count(), time.minutes().count(),
...
BOOST_AUTO_TEST_CASE(test_later_than_timestamp)
{
  ...
  BOOST_CHECK_NE(earlier_timestamp.time_since_epoch().count(), later_timestamp.time_since_epoch().count());
