...
TimePoint timestamp_to_chrono(const v2::TimeStamp& ts)
{
  ...
  return date::sys_days(date::year(ts.year()) / date::month(ts.month()) / date::day(ts.day())) +
      std::chrono::hours(ts.hour()) + std::chrono::minutes(ts.minute()) + std::chrono::seconds(ts.second());
}

bool is_empty_timestamp(const v2::TimeStamp& ts)
{
  return (ts.year() == 0 && ts.month() == 0 && ts.day() == 0 && ts.hour() == 0 && ts.minute() == 0 && ts.second() == 0);
}

TimePoint get_enqueued_time(const v2::TimeStamp* enqueued_time_utc, const v2::TimeStamp* client_time_utc,
