...
void apply_cli_overrides(std::unique_ptr<i_joiner>& joiner, VW::workspace *all, const parser_options &parsed_options)
{
  if(all->options->was_supplied("default_reward")) {
    joiner->set_default_reward(parsed_options.default_reward, true);
  }
...
parser::get_external_parser(VW::workspace *all, const parser_options &parsed_options) {
...
std::unique_ptr<VW::workspace> initialize_with_binary_parser(std::unique_ptr<config::options_i> options)
{
    ...
    if (parsed_options.is_enabled()) {
      auto external_parser =
          VW::external::parser::get_external_parser(all.get(), parsed_options);
      ...
      // of the workspace.
      all->metric_output_hooks.push_back(
          [external_parser_ptr](VW::metric_sink &metric_list) {
            external_parser_ptr->persist_metrics(metric_list);
          });
      all->custom_parser =
          std::unique_ptr<VW::details::input_parser>(external_parser.release());
