...
watchdog::watchdog(error_callback_fn* error_callback) : _error_callback(error_callback), _trace_logger(nullptr) {}

watchdog::~watchdog() { stop(); }

void watchdog::register_thread(
    std::thread::id const& thread_id, std::string const& thread_name, long long const timeout)
{
  ...
  _thread_infos.emplace(thread_id,
      thread_info{thread_id, thread_name, clock_t::now(), clock_t::now(), std::chrono::milliseconds{timeout}});
...
void watchdog::unregister_thread(std::thread::id const& thread_id)
{
  ...
  if (iterator != _thread_infos.end()) { _thread_infos.erase(iterator); }
}
...
void watchdog::loop()
{
  while (_running.load())
  {
    ...
    // If a thread hasn't checking in during this timeout period it is assumed to be unresponsive and an error is
    // generated.
...

    {
      ...
      for (auto& kv : _thread_infos)
      {
        ...
        {
          if (_error_callback) { failed_thread_names.push_back(thread_info.thread_name); }
...
bool watchdog::has_background_error_been_reported() const { return _unhandled_background_error_occurred.load(); }

void watchdog::set_unhandled_background_error(bool const value) { _unhandled_background_error_occurred.store(value); }
