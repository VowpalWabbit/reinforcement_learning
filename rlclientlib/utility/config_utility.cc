...
const char* regex_code_str(int code)
{
  switch (code)
  {
    ...
    case std::regex_constants::error_badrepeat:
      return "The expression contained a repeat specifier(one of * ? +{) that was not preceded by a valid regular "
             "expression.";
    case std::regex_constants::error_complexity:
      return "The complexity of an attempted match against a regular expression exceeded a pre - set level.";
    case std::regex_constants::error_stack:
      return "There was insufficient memory to determine whether the regular expression could match the specified "
             "character sequence.";
    default:
...
int parse_eventhub_conn_str(const std::string& conn_str, std::string& host, std::string& name,
    std::string& access_key_name, std::string& access_key, i_trace* trace, api_status* status)
{
  try
  {
    const std::regex regex_eh_connstr(
  ...
  {
    RETURN_ERROR_LS(trace, status, eh_connstr_parse_error)
        << conn_str << ", regex_error: " << regex_code_str(e.code()) << ", details: " << e.what();
  }
}

int set_eventhub_config(
    const std::string& conn_str, const std::string& cfg_root, configuration& cc, i_trace* trace, api_status* status)
{
  std::string host;
  std::string name;
...
int translate_property(
    std::string prop_name, const char* string_value, configuration& cc, i_trace* trace, api_status* status)
{
  ...
  static const std::unordered_map<std::string, std::string> legacy_translation_mapping = {
      {"ApplicationID", name::APP_ID}, {"applicationID", name::APP_ID}, {"ModelBlobUri", name::MODEL_BLOB_URI},
      {"modelBlobUri", name::MODEL_BLOB_URI}, {"SendHighMaterMark", name::INTERACTION_SEND_HIGH_WATER_MARK},
      {"sendHighMaterMark", name::INTERACTION_SEND_HIGH_WATER_MARK},
      {"SendBatchIntervalMs", name::INTERACTION_SEND_BATCH_INTERVAL_MS},
      {"sendBatchIntervalMs", name::INTERACTION_SEND_BATCH_INTERVAL_MS},
      {"InitialExplorationEpsilon", name::INITIAL_EPSILON}, {"initialExplorationEpsilon", name::INITIAL_EPSILON},
  ...
  static const std::unordered_map<std::string, std::string> parsed_translation_mapping = {
      ...
      {"EventHubInteractionConnectionString", "interaction"}, {"eventHubInteractionConnectionString", "interaction"},
      {"EventHubObservationConnectionString", "observation"}, {"eventHubObservationConnectionString", "observation"}};

  static const std::unordered_set<std::string> deprecated = {"QueueMaxSize", "queueMaxSize"};

  const auto legacy_it = legacy_translation_mapping.find(prop_name);
  if (legacy_it != legacy_translation_mapping.end()) { prop_name = legacy_it->second; }
...
// TODO: There was an attempt previously to make this work using case-insensitive hashing in std:map (and possibly
...
int create_from_json(const std::string& config_json, configuration& cc, i_trace* trace, api_status* status)
{
  ...
  try
  {
    ...
    if (obj.HasParseError())
    {
      RETURN_ERROR_LS(trace, status, json_parse_error)
          << "JSON parse error: " << rj::GetParseError_En(obj.GetParseError()) << " (" << obj.GetErrorOffset() << ")"
  ...
  for (auto const& prop_pair : jsonObj)
  {
    ...
    {
      RETURN_ERROR_LS(trace, status, json_parse_error)
          << "Invalid json type found: " << prop_value.GetType()
          << "\nFailed to initialize rlclientlib, please check config file for accuracy.";
    }
