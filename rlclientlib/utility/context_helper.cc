...
namespace reinforcement_learning { namespace utility {
  ...
  const auto df = sutil::to_string_t("_df");
  ...
  int get_event_ids(std::vector<std::pair<size_t, std::string>>& event_ids, const char *context, i_trace* trace, api_status* status) {
    try {
      const auto scontext = sutil::to_string_t(std::string(context));
      auto json_obj = web::json::value::parse(scontext);
      if ( json_obj.has_array_field(df) ) {
        auto const arr = json_obj.at(df).as_array();
        for (int i = 0; i < arr.size(); i++) {
          auto current = arr.at(i);
          if(current.has_string_field(event_id))
          {
            auto event_id_string = current.at(i).as_string();
            event_ids.emplace_back(i, std::string{ event_id_string.begin(), event_id_string.end() });
          }
        }
        return reinforcement_learning::error_code::success;
      }
      RETURN_ERROR_LS(trace, status, json_no_decisions_found);
    }
    catch ( const std::exception& e ) {
      RETURN_ERROR_LS(trace, status, json_parse_error) << e.what();
    }
  ...
  int get_decision_count(size_t& count, const char *context, i_trace* trace, api_status* status) {
    try {
      const auto scontext = sutil::to_string_t(std::string(context));
      auto json_obj = web::json::value::parse(scontext);
      if ( json_obj.has_array_field(df) ) {
