...
events_counter_status get_counter_status(const utility::configuration& config, const char* section)
{
  ...
  const char* sender_implmentation =
      get_str(config, section, name::SENDER_IMPLEMENTATION, get_default_sender_implmentation(section));
...
async_batcher_config get_batcher_config(const configuration& config, const char* section)
{
  ...
  res.batch_content_encoding = config.get_bool(section, name::USE_DEDUP, false) ? value::CONTENT_ENCODING_DEDUP
                                                                                : value::CONTENT_ENCODING_IDENTITY;
