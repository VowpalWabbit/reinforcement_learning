...
int eventhub_http_authorization::check_authorization_validity_generate_if_needed(api_status* status)
{
  ...
  {
    RETURN_IF_FAIL(generate_authorization_string(now, _shared_access_key, _shared_access_key_name, _eventhub_host,
        _eventhub_name, _authorization, _valid_until, status, _trace));
  }
  return error_code::success;
}

int eventhub_http_authorization::generate_authorization_string(std::chrono::seconds now,
    const std::string& shared_access_key, const std::string& shared_access_key_name, const std::string& eventhub_host,
    const std::string& eventhub_name, std::string& authorization_string /* out */, long long& valid_until /* out */,
    api_status* status, i_trace* trace)
{
  ...
  const auto encoded_uri =
      conversions::to_utf8string(web::uri::encode_data_string(conversions::to_string_t(resource_stream.str())));
  ...
  authorization_stream << "SharedAccessSignature sr=" << encoded_uri
                       << "&sig=" << conversions::to_utf8string(encoded_digest) << "&se=" << valid_until
