...
namespace reinforcement_learning {
  int header_authorization::init(const utility::configuration& config, api_status* status, i_trace* trace) {
    ...
    _api_key = api_key;
    #ifdef _WIN32
    _http_api_header_key_name = u::conversions::utf8_to_utf16(config.get(name::HTTP_API_HEADER_KEY_NAME, value::HTTP_API_DEFAULT_HEADER_KEY_NAME));
    #else
  ...
  int header_authorization::insert_authorization_header(http_headers& headers, api_status* status) {
