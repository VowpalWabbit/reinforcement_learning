...
public:
  ...
  explicit periodic_background_proc(const int interval_ms, utility::watchdog& watchdog, std::string const& proc_name,
...
periodic_background_proc<BgProc>::periodic_background_proc(
    const int interval_ms, watchdog& watchdog, std::string const& proc_name, error_callback_fn* perror_cb)
...
{
  if (bgproc == nullptr) { RETURN_ERROR_LS(nullptr, status, invalid_argument) << " (BGProc)"; }
...
void periodic_background_proc<BGProc>::time_loop()
{
  ...
  do
  {
    ...
    if (_proc->run_iteration(&status) != error_code::success) { ERROR_CALLBACK(_perror_cb, status); }
