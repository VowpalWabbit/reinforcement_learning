...
namespace reinforcement_learning {
  namespace utility {
    ...
    data_buffer::value_type* data_buffer::raw_begin() {
      return _buffer.data();
    }

    size_t data_buffer::get_body_beginoffset() const {
      return _body_beginoffset;
    }

    int data_buffer::set_body_beginoffset(size_t beginoffset) {
      if (beginoffset > _buffer.size() || beginoffset < _preamble_size) {
    ...
    size_t data_buffer::get_body_endoffset() const {
      return _body_endoffset;
    }
    ...
    size_t data_buffer::buffer_filled_size() const {
      return preamble_size() + body_filled_size();
    }

    data_buffer::value_type* data_buffer::body_begin() {
      return _buffer.data() + _body_beginoffset;
    }

    size_t data_buffer::body_filled_size() const {
        return _body_endoffset - _body_beginoffset;
    }

    size_t data_buffer::body_capacity() const {
      return _buffer.size() - _preamble_size;
    }
    ...
    void data_buffer::resize_body_region(size_t size) {
      ...
      if (_body_endoffset >= (_buffer.size()))
        _body_endoffset = _buffer.size() - 1;
      if (_body_beginoffset >= (_buffer.size()))
        _body_beginoffset = _buffer.size() - 1;
    }

    data_buffer::value_type* data_buffer::preamble_begin() {
