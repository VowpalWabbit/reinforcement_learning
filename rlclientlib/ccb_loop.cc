...
int ccb_loop::request_decision(str_view context_json, unsigned int flags, decision_response& resp, api_status* status)
{
  INIT_CHECK();
  return _pimpl->request_decision(string_view(context_json.str, context_json.size), flags, resp, status);
}

int ccb_loop::request_decision(str_view context_json, decision_response& resp, api_status* status)
{
  INIT_CHECK();
...
int ccb_loop::request_multi_slot_decision(
    str_view event_id, str_view context_json, unsigned int flags, multi_slot_response& resp, api_status* status)
{
  INIT_CHECK();
...
int ccb_loop::request_multi_slot_decision(
    str_view event_id, str_view context_json, multi_slot_response& resp, api_status* status)
{
  return request_multi_slot_decision(event_id, context_json, action_flags::DEFAULT, resp, status);
}

int ccb_loop::request_multi_slot_decision(
    str_view context_json, unsigned int flags, multi_slot_response& resp, api_status* status)
{
  INIT_CHECK();
...
int ccb_loop::request_multi_slot_decision(str_view context_json, multi_slot_response& resp, api_status* status)
{
  return request_multi_slot_decision(context_json, action_flags::DEFAULT, resp, status);
}

int ccb_loop::request_multi_slot_decision(str_view event_id, str_view context_json, unsigned int flags,
...
int ccb_loop::request_multi_slot_decision(
    str_view event_id, str_view context_json, multi_slot_response_detailed& resp, api_status* status)
{
  return request_multi_slot_decision(event_id, context_json, action_flags::DEFAULT, resp, status);
}

int ccb_loop::request_multi_slot_decision(
    str_view context_json, unsigned int flags, multi_slot_response_detailed& resp, api_status* status)
{
  INIT_CHECK();
  return _pimpl->request_multi_slot_decision(
      string_view(context_json.str, context_json.size), flags, resp, ccb_loop::default_baseline_vector, status);
...
int ccb_loop::request_multi_slot_decision(str_view context_json, multi_slot_response_detailed& resp, api_status* status)
{
  return request_multi_slot_decision(context_json, action_flags::DEFAULT, resp, status);
}
...
int ccb_loop::report_outcome(str_view primary_id, int secondary_id, str_view outcome, api_status* status)
{
  INIT_CHECK();
  return _pimpl->report_outcome(primary_id.str, secondary_id, outcome.str, status);
}

int ccb_loop::report_outcome(str_view primary_id, int secondary_id, float outcome, api_status* status)
{
  INIT_CHECK();
  return _pimpl->report_outcome(primary_id.str, secondary_id, outcome, status);
}

int ccb_loop::report_outcome(str_view primary_id, str_view secondary_id, str_view outcome, api_status* status)
{
  INIT_CHECK();
  return _pimpl->report_outcome(primary_id.str, secondary_id.str, outcome.str, status);
}

int ccb_loop::report_outcome(str_view primary_id, str_view secondary_id, float outcome, api_status* status)
{
  INIT_CHECK();
