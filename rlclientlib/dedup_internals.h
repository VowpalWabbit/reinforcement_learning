...
public:
  ...
  void update(float new_value) { _current = (1 - _weight) * _current + (_weight * new_value); }
...
public:
  ...
  template <typename I>
  int get_all_values(I start, I end, generic_event::object_list_t& action_ids, std::vector<string_view>& action_values,
      api_status* status);
...
template <typename I>
int dedup_state::get_all_values(I start, I end, generic_event::object_list_t& action_ids,
    std::vector<string_view>& action_values, api_status* status)
{
