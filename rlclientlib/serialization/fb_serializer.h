...
struct fb_event_serializer<ranking_event>
{
  ...
  static size_t size_estimate(const ranking_event& evt)
  {
    return evt.get_event_id().size() + evt.get_action_ids().size() * sizeof(evt.get_action_ids()[0]) +
        ...
        sizeof(evt.get_client_time_gmt());
  }
      ...
      flatbuffers::Offset<fb_event_t>& ret_val, api_status* status)
  {
    ...
    TimeStamp client_ts(ts.year, ts.month, ts.day, ts.hour, ts.minute, ts.second, ts.sub_second);
    ...
    ret_val =
        CreateRankingEvent(builder, event_id_offset, evt.get_defered_action(), action_ids_vector_offset, context_offset,
            probabilities_vector_offset, model_id_offset, evt.get_pass_prob(), meta_id_offset, learning_mode_type);
...
struct fb_event_serializer<decision_ranking_event>
{
  ...
  static size_t size_estimate(const decision_ranking_event& evt)
  {
    ...
    estimate += evt.get_context().size() + evt.get_model_id().size() + sizeof(evt.get_defered_action()) +
      ...
      flatbuffers::Offset<fb_event_t>& ret_val, api_status* status)
  {
    ...
    {
      slots.push_back(CreateSlotEvent(builder, builder.CreateString(decision_slot_ids[i]),
          builder.CreateVector(action_ids[i]), builder.CreateVector(probabilities[i])));
    }
    ...
    ret_val = CreateDecisionEvent(builder, context_offset, slots_offset, model_id_offset, evt.get_pass_prob(),
        evt.get_defered_action(), meta_id_offset);
    return error_code::success;
  }
};
...
struct fb_event_serializer<multi_slot_decision_event>
{
      ...
      flatbuffers::Offset<fb_event_t>& ret_val, api_status* status)
  {
    ...
    {
      slots.push_back(
          CreateSlatesSlotEvent(builder, builder.CreateVector(action_ids[i]), builder.CreateVector(probabilities[i])));
    }
    const auto slots_offset = builder.CreateVector(slots);
    ...
    ret_val = CreateSlatesEvent(builder, event_id_offset, context_offset, slots_offset, model_id_offset,
        evt.get_pass_prob(), evt.get_defered_action(), meta_id_offset);
    return error_code::success;
  }
};
      ...
      flatbuffers::Offset<fb_event_t>& retval, api_status* status)
  {
    ...
    switch (evt.get_outcome_type())
    {
      case outcome_event::outcome_type_string:
      {
        ...
        retval = CreateOutcomeEventHolder(
            builder, event_id, evt.get_pass_prob(), OutcomeEvent_StringEvent, str_event, meta_id_offset);
        break;
      }
      case outcome_event::outcome_type_numeric:
      {
        ...
        retval = CreateOutcomeEventHolder(
            builder, event_id, evt.get_pass_prob(), OutcomeEvent_NumericEvent, number_event, meta_id_offset);
        break;
      }
      case outcome_event::outcome_type_action_taken:
      {
        ...
        retval = CreateOutcomeEventHolder(
            builder, event_id, evt.get_pass_prob(), OutcomeEvent_ActionTakenEvent, action_taken_event, meta_id_offset);
        break;
      }
      default:
      {
...
struct fb_collection_serializer
{
  ...
  fb_collection_serializer(buffer_t& buffer, const char* content_encoding)
      ...
      , _original_event_count(0)
  {
  }

  fb_collection_serializer(buffer_t& buffer, const char* content_encoding, int /*dummy*/)
      : fb_collection_serializer(buffer, content_encoding)
  {
  }
  ...
  void create_header() { return; }

  void add_header(typename serializer_t::batch_builder_t& batch_builder) { return; }
...
struct fb_event_serializer<generic_event>
{
      ...
      flatbuffers::Offset<fb_event_t>& ret_val, api_status* status)
  {
    ...
    const auto& ts = evt.get_client_time_gmt();
    v2::TimeStamp client_ts(ts.year, ts.month, ts.day, ts.hour, ts.minute, ts.second, ts.sub_second);
    const auto meta_offset = v2::CreateMetadataDirect(builder, evt.get_id(), &client_ts, evt.get_app_id(),
        evt.get_payload_type(), evt.get_pass_prob(), evt.get_encoding());
...
template <>
inline int fb_collection_serializer<outcome_event>::message_id()
{
  return message_type::fb_outcome_event_collection;
}

template <>
inline int fb_collection_serializer<decision_ranking_event>::message_id()
{
  return message_type::fb_decision_event_collection;
}

template <>
inline int fb_collection_serializer<multi_slot_decision_event>::message_id()
{
  return message_type::fb_slates_event_collection;
}

template <>
inline int fb_collection_serializer<ranking_event>::message_id()
{
  return message_type::fb_ranking_learning_mode_event_collection;
}

template <>
inline int fb_collection_serializer<generic_event>::message_id()
{
  return message_type::fb_generic_event_collection;
}
