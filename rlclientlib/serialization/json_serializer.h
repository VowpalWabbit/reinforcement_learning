...
struct json_event_serializer<ranking_event>
{
  static int serialize(ranking_event& evt, std::ostream& buffer, api_status* status)
  {
    ...
    if (evt.get_defered_action()) { buffer << R"(,"DeferredAction":true)"; }
    ...
    if (evt.get_pass_prob() < 1) { buffer << R"(,"pdrop":)" << (1 - evt.get_pass_prob()); }
    buffer << R"(})";
...
struct json_collection_serializer
{
  ...
  json_collection_serializer(buffer_t& buffer, const char* content_encoding)
      : _buffer(buffer), _streambuf{&_buffer}, _ostream{&_streambuf}
  {
    _ostream << std::unitbuf;
  }

  json_collection_serializer(buffer_t& buffer, const char* content_encoding, int /*dummy*/)
      : json_collection_serializer(buffer, content_encoding)
  {
  }
  ...
  uint64_t size() const { return _buffer.body_filled_size(); }
...
template <>
inline int json_collection_serializer<outcome_event>::message_id()
{
  return message_type::json_outcome_event_collection;
}

template <>
inline int json_collection_serializer<ranking_event>::message_id()
{
  return message_type::json_ranking_event_collection;
}
}  // namespace logger
}  // namespace reinforcement_learning
