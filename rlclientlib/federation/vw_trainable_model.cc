...
int trainable_vw_model::learn(VW::workspace& example_ws, VW::multi_ex& examples, api_status* status)
{
  try
  {
    ...
    for (auto example : examples)
    {
      ...
      VW::parsers::cache::write_example_to_cache(io_writer, example,
          example_ws.parser_runtime.example_parser->lbl_parser, example_ws.runtime_state.parse_mask, temp_buffer);
      io_writer.flush();
