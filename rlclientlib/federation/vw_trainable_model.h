...
public:
  ...
  static int create_trainable_vw_model(std::unique_ptr<trainable_vw_model>& output,
      const utility::configuration& config, std::unique_ptr<i_joined_log_provider>&& eud_joiner,
  ...
  int learn(api_status* status = nullptr);
...
private:
  ...
  trainable_vw_model(std::unique_ptr<i_joined_log_provider>&& eud_joiner, std::string command_line,
      std::string problem_type, std::string learning_mode, std::string reward_function);

  std::string _command_line;
  std::unique_ptr<i_joined_log_provider> _eud_joiner;
  std::unique_ptr<example_joiner> _example_joiner;
