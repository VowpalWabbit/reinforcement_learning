...
public:
  ...
  : VW::io::reader(true),
  _buffer(std::move(buffer)), _read_head(buffer.data())
  {
...
{
    return reinforcement_learning::messages::flatbuff::v2::TimeStamp(ts.year, ts.month, ts.day, ts.hour, ts.minute, ts.second, ts.sub_second);
}
...
int emit_regular_message(std::vector<uint8_t>& output, flatbuffers::FlatBufferBuilder& fbb, flatbuffers::Offset<reinforcement_learning::messages::flatbuff::v2::JoinedPayload> joined_payload)
{
  // TODO consider doing this a safer way
        ...
        if (observations != _observations.end())
        {
          for (const auto& observation : observations->second)
          {
            ...
            if (observation_time <= reward_cutoff)
            {
              ...
              events.push_back(reinforcement_learning::messages::flatbuff::v2::CreateJoinedEventDirect(fbb, &std::get<1>(observation), &ts));
            }
          }
