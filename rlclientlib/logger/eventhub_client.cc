...
eventhub_client::http_request_task::http_request_task(
    ...
    const buffer& post_data, size_t max_retries,
...
eventhub_client::http_request_task::send_request(size_t try_count) {
  // TODO: Use a custom thread pool to avoid the overhead of std::async thread
  // creation.
  return std::async(std::launch::async, [this, try_count]() {
    ...
    // If the response is not the expected code then it has failed. Retry if
    // possible otherwise report background error.
    if (code != http_response::status::CREATED) {
      // TODO: Handle retrying as part of HTTP interface, to avoid repeated
      // reading of the body _post_data.

      // Stop condition of recurison.
      if (try_count < _max_retries) {
        TRACE_ERROR(_trace, "HTTP request failed, retrying...");

        // Yes, recursively send another request inside this one. If a
        // subsequent request returns success we are good, otherwise the failure
        // will propagate.
        return send_request(try_count + 1).get();
      }

      auto msg = u::concat("(expected 201): Found ", code, ", failed after ",
                           try_count, " retries.");
      api_status::try_update(&status, error_code::http_bad_status_code,
                             msg.c_str());
...
int eventhub_client::http_request_task::join() {
  ...
  // The task may have failed but was reported with the callback. This
  // function's primary purpose is to block if the task is not yet complete.
...
int eventhub_client::pop_task(api_status* status) {
  // This function must be under a lock as there is a delay between popping from
  // the queue and joining the task, but it should essentially be atomic.
  ...
  try {
    ...
    // Ignore if there is an exception surfaced as this should have been handled
    // in the continuation.
    TRACE_WARN(_trace,
               "There should not be an exception raised in pop_task function.");
  }
...
int eventhub_client::v_send(const buffer& post_data, api_status* status) {
  ...
  try {
    ...
    std::unique_ptr<http_request_task> request_task(new http_request_task(
        _client.get(), _eventhub_host, auth_str, post_data, _max_retries,
        _error_callback, _trace));
...
eventhub_client::eventhub_client(
    std::unique_ptr<i_http_client> client, const std::string& host,
    const std::string& key_name, const std::string& key,
    const std::string& name, size_t max_tasks_count, size_t max_retries,
    i_trace* trace, error_callback_fn* error_callback)
