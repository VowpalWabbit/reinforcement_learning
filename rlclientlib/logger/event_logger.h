...
class ccb_logger : public event_logger<decision_ranking_event> {
  public:
    ccb_logger(const utility::configuration& c, i_message_sender* sender, utility::watchdog& watchdog, error_callback_fn* perror_cb = nullptr)
      : event_logger(
        sender,
        c.get_int(name::DECISIONS_SEND_HIGH_WATER_MARK, 198 * 1024),
        c.get_int(name::DECISIONS_SEND_BATCH_INTERVAL_MS, 1000),
        c.get_int(name::DECISIONS_SEND_QUEUE_MAX_CAPACITY_KB, 16 * 1024) * 1024,
        c.get(name::QUEUE_MODE, "DROP"),
        watchdog,
        perror_cb)
    {}

    int log_decisions(std::vector<const char*>& event_ids, const char* context, unsigned int flags, const ranking_responses& response, api_status* status);
  };
