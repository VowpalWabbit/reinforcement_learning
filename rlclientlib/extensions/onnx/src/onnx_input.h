...
{
  return check_array_packing<element_t>(bytes, element_count) && (element_count == expected_element_count);
}
...
public:
  ...
  int allocate_inputs(
      std::vector<Ort::Value>& result, const Ort::MemoryInfo& allocator_info, api_status* status = nullptr) const;

  inline size_t input_count() const { return count(); }

  inline size_t count() const { return _inputs.size(); }
...
int read_tensor_notation(reinforcement_learning::string_view tensor_notation, onnx_input_builder& input_context,
