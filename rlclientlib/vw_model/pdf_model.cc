...
pdf_model::pdf_model(i_trace* trace_logger, const utility::configuration& /*unused*/)
    : _trace_logger(trace_logger), _vw(new safe_vw("--json --quiet --cb_adf", input_serialization::dsjson))
{
}
...
int pdf_model::choose_continuous_action(
...
// Not supported.
int pdf_model::request_decision(const std::vector<const char*>& event_ids, string_view features,
...
int pdf_model::request_multi_slot_decision(const char* event_id, const std::vector<std::string>& slot_ids,
