...
namespace reinforcement_learning {
  event::event(string_view seed_id, const timestamp& ts, float pass_prob)
    : _seed_id(seed_id), _pass_prob(pass_prob), _client_time_gmt(ts), _event_index(0) {}
  ...
  uint64_t event::get_event_index() const { return _event_index; }
  ...
  ranking_event::ranking_event(string_view event_id, bool deferred_action, float pass_prob, string_view context,
  ...
  ranking_event ranking_event::choose_rank(string_view event_id, string_view context, unsigned int flags,
  ...
  decision_ranking_event::decision_ranking_event(const std::vector<string_view>& event_ids, bool deferred_action, float pass_prob, string_view context,
  ...
  decision_ranking_event decision_ranking_event::request_decision(const std::vector<string_view>& event_ids, string_view context, unsigned int flags, const std::vector<std::vector<uint32_t>>& action_ids, const std::vector<std::vector<float>>& pdfs, const std::string& model_version, const timestamp& ts, float pass_prob) {
    return decision_ranking_event(event_ids, flags & action_flags::DEFERRED, pass_prob, context, action_ids, pdfs, model_version, ts);
  }

  multi_slot_decision_event::multi_slot_decision_event(string_view event_id, bool deferred_action, float pass_prob, string_view context, const std::vector<std::vector<uint32_t>>& action_ids, const std::vector<std::vector<float>>& pdfs, const std::string& model_version, const timestamp& ts)
  : event(event_id, ts, pass_prob),
  _event_id(event_id),
  _deferred_action(deferred_action),
  _action_ids_vector(action_ids),
  _probilities_vector(pdfs),
  ...
  multi_slot_decision_event multi_slot_decision_event::request_decision(string_view event_id, string_view context, unsigned int flags, const std::vector<std::vector<uint32_t>>& action_ids, const std::vector<std::vector<float>>& pdfs, const std::string& model_version, const timestamp& ts, float pass_prob) {
  ...
  outcome_event::outcome_event(string_view event_id, float pass_prob, string_view outcome, bool action_taken, const timestamp& ts)
    : event(event_id, ts, pass_prob), _outcome(outcome), _float_outcome(0.0f), _action_taken(action_taken) { }

  outcome_event::outcome_event(string_view event_id, float pass_prob, float outcome, bool action_taken, const timestamp& ts)
    : event(event_id, ts, pass_prob), _outcome(""), _float_outcome(outcome), _action_taken(action_taken) { }

  outcome_event outcome_event::report_outcome(string_view event_id, string_view outcome, const timestamp& ts, float pass_prob) {
    outcome_event evt(event_id, pass_prob, outcome, false, ts);
    evt._outcome_type = outcome_type_string;
    return evt;
  }

  outcome_event outcome_event::report_outcome(string_view event_id, float outcome, const timestamp& ts, float pass_prob) {
    outcome_event evt(event_id, pass_prob, outcome, false, ts);
    evt._outcome_type = outcome_type_numeric;
    return evt;
  }

  outcome_event outcome_event::report_action_taken(string_view event_id, const timestamp& ts, float pass_prob) {
