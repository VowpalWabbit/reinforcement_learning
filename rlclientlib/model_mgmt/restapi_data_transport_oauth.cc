#include "restapi_data_transport_oauth.h"
...
{
restapi_data_transport_oauth::restapi_data_transport_oauth(i_http_client* httpcli, i_trace* trace,
    std::unique_ptr<i_oauth_credentials_provider>&& cred_provider, std::string scope)
    : _httpcli(httpcli), _datasz{0}, _trace{trace}, _headerimpl(std::move(cred_provider), std::move(scope))
{
}
restapi_data_transport_oauth::restapi_data_transport_oauth(std::unique_ptr<i_http_client>&& httpcli,
    utility::configuration cfg, model_source model_source, i_trace* trace,
    std::unique_ptr<i_oauth_credentials_provider>&& cred_provider, std::string scope)
    ...
    , _datasz{0}
    , _trace{trace}
    , _headerimpl(std::move(cred_provider), std::move(scope))
{
}
...
int restapi_data_transport_oauth::get_data_info(
...
int restapi_data_transport_oauth::get_data(model_data& ret, api_status* status)
{
