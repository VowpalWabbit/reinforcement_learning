...
namespace reinforcement_learning {
	namespace model_management {
		...
		{}
		restapi_data_transport::restapi_data_transport(i_http_client* httpcli, ::web::http::http_headers& header, bool usingAPI, i_trace* trace)
			: _httpcli(httpcli), _header(header), _usingAPI(usingAPI), _datasz{ 0 }, _trace{ trace }
		{}
		...
		int restapi_data_transport::get_data(model_data& ret, api_status* status) {
			...
			request.headers() = _header;
			// Build request URI and start the request.
			auto request_task = _httpcli->request(request)
