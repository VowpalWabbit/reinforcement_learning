...
namespace reinforcement_learning
{
  namespace model_management
  {
    ...
    */
    ...
    int restapi_data_transport::get_data_info(std::string &last_modified, uint64_t &sz, api_status *status)
    {
      try
      {
        const auto response = _httpcli->request(http_request(http_method::HEAD));

        // TODO: Use status_is_ok() instead of checking 200 directly.
        if (response->status_code() != 200)
          RETURN_ERROR_ARG(_trace, status, http_bad_status_code, _httpcli->get_url());

        // TODO: Simplify handling of error codes.
        const auto err = response->last_modified(last_modified);
        if (err == error_code::last_modified_not_found)
        {
          RETURN_ERROR_ARG(_trace, status, last_modified_not_found, _httpcli->get_url());
        }
        else if (err == error_code::last_modified_invalid)
        {
    ...
    int restapi_data_transport::get_data(model_data &ret, api_status *status)
    {
      ...
      try
      {
        ...
        // TODO: Handle _trace and status, similar to get_data_info.
