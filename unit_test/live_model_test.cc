...
BOOST_AUTO_TEST_CASE(schema_v1_with_bad_use_dedup)
{
  ...
  r::api_status status;
  r::live_model_cb ds = create_mock_live_model<r::live_model_cb>(config, nullptr, nullptr, nullptr);
...
BOOST_AUTO_TEST_CASE(live_model_request_continuous_action)
{
  ...
  r::live_model_ca ds = create_mock_live_model<r::live_model_ca>(config, nullptr, &reinforcement_learning::model_factory, nullptr);
  BOOST_CHECK_EQUAL(ds.init(&status), err::success);
...
BOOST_AUTO_TEST_CASE(live_model_request_decision)
{
  ...
  // Create the ds live_model, and initialize it with the config
  r::live_model_ccb ds = create_mock_live_model<r::live_model_ccb>(config, nullptr, &reinforcement_learning::model_factory, nullptr);
  BOOST_CHECK_EQUAL(ds.init(&status), err::success);
...
BOOST_AUTO_TEST_CASE(live_model_ranking_request_pdf_passthrough)
{
  ...
  r::live_model_cb model = create_mock_live_model<r::live_model_cb>(config, &r::data_transport_factory, &r::model_factory, nullptr);
...
BOOST_AUTO_TEST_CASE(live_model_ranking_request_pdf_passthrough_underscore_p)
{
  ...
  // Background refresh introduces a timing issue where the model might not have updated properly before the
  // choose_rank() call.
  config.set(r::name::MODEL_BACKGROUND_REFRESH, "false");
  ...
  // Request ranking
  constexpr auto JSON_PDF =
      R"({"Shared":{"t":"abc"}, "_multi":[{"Action":{"c":1}},{"Action":{"c":2}}],"_p":[0.4, 0.6]})";
...
BOOST_AUTO_TEST_CASE(live_model_outcome_with_secondary_id_and_v1)
{
  ...
  // create a ds live_model, and initialize with configuration
  r::live_model_ccb ds = create_mock_live_model<r::live_model_ccb>(config, nullptr, model_factory.get(), nullptr);
  ...
  BOOST_CHECK_EQUAL(
      ds.report_outcome("event_id", "valid_secondary", "valid_outcome", &status), err::protocol_not_supported);
  BOOST_CHECK_EQUAL(ds.report_outcome("event_id", "", 1.5f, &status),
      err::invalid_argument);  // we fail the input check before we check for protocol versin
}
...
void algo_error_func(const r::api_status&, algo_server* ph) { ph->ml_error_handler(); }
...
BOOST_AUTO_TEST_CASE(typesafe_err_callback)
{
  ...
  {
    // create a ds live_model, and initialize with configuration
    r::live_model_cb ds(config, algo_error_func, &the_server, &r::trace_logger_factory, data_transport_factory.get(),
...
BOOST_AUTO_TEST_CASE(live_model_mocks)
{
  ...
  config.set(r::name::EH_TEST, "true");
  {
    r::live_model_cb model =
        create_mock_live_model<r::live_model_cb>(config, data_transport_factory.get(), model_factory.get(), sender_factory.get());
...
BOOST_AUTO_TEST_CASE(live_model_background_refresh)
{
  ...
  r::live_model_cb model = create_mock_live_model<r::live_model_cb>(config, nullptr, nullptr, nullptr);
...
BOOST_AUTO_TEST_CASE(live_model_no_background_refresh_failure)
{
  ...
  r::live_model_cb model = create_mock_live_model<r::live_model_cb>(config, data_transport_factory.get());
...
BOOST_AUTO_TEST_CASE(live_model_logger_receive_data)
{
  ...
  {
    r::live_model_cb model =
        create_mock_live_model<r::live_model_cb>(config, data_transport_factory.get(), model_factory.get(), logger_factory.get());
...
BOOST_AUTO_TEST_CASE(populate_response_same_size_test)
{
  ...
  BOOST_CHECK_EQUAL(
      populate_response(action_ids, pdfs, event_ids, std::move(model_id), resp, nullptr, &status), err::success);
...
BOOST_AUTO_TEST_CASE(populate_response_different_size_test)
{
  ...
  BOOST_CHECK_EQUAL(populate_response(action_ids, pdfs, event_ids, std::move(model_id), resp, nullptr, &status),
...
BOOST_AUTO_TEST_CASE(populate_multi_slot_response_same_size_test)
{
  ...
  BOOST_CHECK_EQUAL(populate_multi_slot_response_detailed(
                        action_ids, pdfs, std::move(event_id), std::move(model_id), slot_ids, resp, nullptr, &status),
      err::success);
...
BOOST_AUTO_TEST_CASE(slates_explore_only_mode)
{
  ...
  r::api_status status;
  r::live_model_slates model(config);
  BOOST_CHECK_EQUAL(model.init(&status), err::success);
...
BOOST_AUTO_TEST_CASE(live_model_ccb_and_v2)
{
  ...
  config.set(r::name::PROTOCOL_VERSION, "2");
  r::live_model_ccb ds2 = create_mock_live_model<r::live_model_ccb>(config, nullptr, &reinforcement_learning::model_factory, nullptr);
...
BOOST_AUTO_TEST_CASE(live_model_using_endpoint_failure_no_uri)
{
  ...
  r::api_status status;
  std::unique_ptr<reinforcement_learning::live_model_cb> _rl =
      std::unique_ptr<r::live_model_cb>(new r::live_model_cb(config, nullptr));
