...
std::unique_ptr<fakeit::Mock<m::i_model>> get_mock_model() {
  ...
  const std::function<int(const char*, std::vector<std::vector<int>>&, std::vector<std::vector<float>>&, std::string&, r::api_status*)> request_decision_fn =
    [](const char*, std::vector<std::vector<int>>&, std::vector<std::vector<float>>&, std::string& model_version, r::api_status*) {
    model_version = "model_id";
    return r::error_code::success;
  };
