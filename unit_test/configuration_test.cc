...
BOOST_AUTO_TEST_CASE(configuration_parse_eventhub_connection_string)
{
  ...
  BOOST_CHECK_EQUAL(util::config::create_from_json(json, config), err::success);
  BOOST_CHECK_EQUAL(
      config.get(reinforcement_learning::name::INTERACTION_EH_HOST, "DefaultValue"), "<ingest>.servicebus.windows.net");
  BOOST_CHECK_EQUAL(config.get(reinforcement_learning::name::INTERACTION_EH_NAME, "DefaultValue"), "interaction");
  BOOST_CHECK_EQUAL(
      config.get(reinforcement_learning::name::INTERACTION_EH_KEY_NAME, "DefaultValue"), "RootManageSharedAccessKey");
...
BOOST_AUTO_TEST_CASE(configuration_parse_episode_interaction_observation_config)
{
  ...
  BOOST_CHECK_EQUAL(
      config.get(reinforcement_learning::name::INTERACTION_HTTP_API_HOST, "DefaultValue"), "http://localhost:8080");
...
BOOST_AUTO_TEST_CASE(configuration_parse_malformed_eventhub_connection_string_expect_error)
{
  ...
  const auto json1 =
      R"({"EventHubInteractionConnectionString": "<ingest>.servicebus.windows.net/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=<SAKey>;EntityPath=interaction"})";
  const auto json2 =
      R"({"EventHubInteractionConnectionString": "Endpoint=sb://<ingest>.servicebus.windows.net/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=<SAKey>"})";
  const auto json3 =
      R"({"EventHubInteractionConnectionString": "Endpoint=sb://<ingest>.servicebus.windows.net/;AccessKeyName=RootManageSharedAccessKey;SharedAccessKey=<SAKey>;EntityPath=interaction"})";
  const auto json4 =
      R"({"EventHubInteractionConnectionString": "Endpoint=mp://<ingest>.servicebus.windows.net/,SharedAccessKeyName=RootManageSharedAccessKey,SharedAccessKey=<SAKey>,EntityPath=interaction"})";
