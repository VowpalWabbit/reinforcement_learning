...
BOOST_AUTO_TEST_CASE(dedup_state_expected_use)
{
  ...
  BOOST_CHECK_EQUAL(
      err::success, state.transform_payload_and_add_objects(payload.c_str(), edited_payload, obj_list, nullptr));
  ...
  BOOST_CHECK_EQUAL(err::success,
      state.get_all_values(batch_actions.begin(), batch_actions.end(), action_ids, action_values, nullptr));
...
BOOST_AUTO_TEST_CASE(dedup_state_and_bad_state)
{
  ...
  r::generic_event::object_list_t action_ids;
  std::vector<r::string_view> action_values;
  BOOST_CHECK_EQUAL(err::compression_error,
      state.get_all_values(batch_actions.begin(), batch_actions.end(), action_ids, action_values, nullptr));
  BOOST_CHECK_EQUAL(
      err::compression_error, state.remove_all_values(batch_actions.begin(), batch_actions.end(), nullptr));
