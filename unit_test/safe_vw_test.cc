...
{
  safe_vw vw((const char*)cb_data_5_model, cb_data_5_model_len, input_serialization::vwjson);
...
{
  safe_vw vw((const char*)cb_data_5_model, cb_data_5_model_len, "--json --quiet", input_serialization::vwjson);
  const auto json = R"({"a":{"0":1,"5":2},"_multi":[{"b":{"0":1}},{"b":{"0":2}},{"b":{"0":3}}]})";
  ...
  safe_vw vw_w_audit((const char*)cb_data_5_model, cb_data_5_model_len, "--json --audit", input_serialization::vwjson);
...
BOOST_AUTO_TEST_CASE(factory_with_cb_model_and_ccb_arguments)
{
  ...
  const safe_vw_factory factory(model_data, vw_commandLine, input_serialization::vwjson);
...
BOOST_AUTO_TEST_CASE(factory_with_initial_model)
{
  ...
  const safe_vw_factory factory(model_data, input_serialization::vwjson);
  versioned_object_pool<safe_vw> pool(factory);

  {
    ...
    pool.update_factory(safe_vw_factory(updated_model, input_serialization::vwjson));
...
BOOST_AUTO_TEST_CASE(factory_with_empty_model)
{
  ...
  const safe_vw_factory factory(empty_data, input_serialization::vwjson);
  versioned_object_pool<safe_vw> pool(factory);
  ...
  // Initial model & rank call
  {
    ...
    pool.update_factory(safe_vw_factory(new_model, input_serialization::vwjson));
    auto vw = pool.get_or_create();
