...
public:
  explicit message_sender(std::vector<std::string>& _items) : items(_items), sender{nullptr} {}
...
public:
  ...
  test_undroppable_event(const std::string& id) : event(id.c_str(), timestamp{}) {}
...
public:
  ...
  test_droppable_event(const std::string& id) : event(id.c_str(), timestamp{}) {}
