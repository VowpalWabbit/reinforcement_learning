...
class person:
    ...
    def get_features(self):
        return '"User":{{"id":"{}","major":"{}","hobby":"{}","favorite_character":"{}"}}'.format(
            self._id, self._major, self._hobby, self._favorite_character
        )
        ...
        while round != self._options.num_rounds:
            try:
                ...
                action_features = (
                    '"_multi":['
                    + ",".join(
                        '{"a":{"topic":"' + action + '"}}' for action in self._actions
                    )
                    + "]"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_rounds",
        help="number of rounds (default: infinity)",
        type=int,
        default=-1,
    )
