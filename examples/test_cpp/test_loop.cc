...
test_loop::test_loop(size_t index, const boost::program_options::variables_map& vm)
  ...
  , experiment_name(generate_experiment_name(vm["experiment_name"].as<std::string>(), threads, vm["features"].as<size_t>(), vm["actions"].as<size_t>(), vm["slots"].as<size_t>(), vm["episode_length"].as<size_t>(), index))
...
std::string test_loop::generate_experiment_name(const std::string& experiment_name_base, size_t threads, size_t features, size_t actions, size_t slots, size_t episode_length, size_t index) const {
  return experiment_name_base + "-t" + std::to_string(threads) + "-x" + std::to_string(features) + "-a" + std::to_string(actions) + "-s" + std::to_string(slots) + "-m" + std::to_string(episode_length) + "-i" + std::to_string(index);
}
...
LoopKind test_loop::get_loop_kind(const boost::program_options::variables_map& vm) const {
  ...
  if (vm["episode_length"].as<size_t>() > 0)
    return LoopKind::MULTISTEP;
...
void test_loop::multistep_loop(size_t thread_id) const {
  ...
  size_t choose_rank_size = 0;
  {
    ...
    const auto event_id = test_inputs.create_event_id(0, 0);
    const std::string warmup_id = "_warmup_" + std::string(event_id.c_str());
    if (rl->request_episodic_decision(warmup_id.c_str(), nullptr, test_inputs.get_context(0, 0), response, episode, &status) != err::success) {
      std::cout << "Warmup has failed. " << status.get_error_msg() << std::endl;
      return;
    }
  ...
  for (controller->restart(); controller->is_running(); controller->iterate()) {
    ...
    if (rl->request_episodic_decision(event_id.c_str(), previous_event_id.empty() ? nullptr : previous_event_id.c_str(), test_inputs.get_context(thread_id, example_id), response, *current_episode, &status) != err::success) {
      std::cout << status.get_error_msg() << std::endl;
      continue;
    }
