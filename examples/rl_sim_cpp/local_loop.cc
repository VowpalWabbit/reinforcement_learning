...
void local_model::set_trace_logger(rl::i_trace* trace_logger) { _trace_logger = trace_logger; }

// Get data causes a "join" to take place.
int local_model::get_data(
    reinforcement_learning::model_management::model_data& data, reinforcement_learning::api_status* status)
{
  ...
  if (!_joiner) { return rl::error_code::success; }
    ...
    const reinforcement_learning::utility::configuration& config, reinforcement_learning::api_status* status)
{
  ...
  {
    _vw_model =
        std::unique_ptr<rl::model_management::vw_model>(new rl::model_management::vw_model(_trace_logger, config));
  }
  std::string cmd_str = config.get(rl::name::MODEL_VW_INITIAL_COMMAND_LINE,
...
int local_model::v_send(const buffer& data, reinforcement_learning::api_status* status)
{
  std::lock_guard<std::mutex> lock(_mutex);
  ...
  if (pre.msg_type == rl::logger::message_type::fb_generic_event_collection)
  {
    ...
    for (auto message : *res->events())
    {
      ...
      const auto* je = flatbuffers::GetRoot<rl::messages::flatbuff::v2::JoinedEvent>(_detached_buffers.back().data());
