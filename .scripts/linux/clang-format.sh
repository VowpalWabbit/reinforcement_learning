...
# The ubuntu:jammy image should have clang-format version 14, which matches what runs in Github Actions
if [[ "$1" == "docker" ]]; then
    DOCKER_CMD="echo 'Installing clang-format in docker container...'; apt update -qq; apt install -qq -y clang-format; cd /reinforcement_learning; ./.scripts/linux/clang-format.sh ${@:2}"
    if command -v podman &> /dev/null; then
        # podman supports --env-host for forwarding all host environment variables
        podman run -it --env-host -v "$REPO_DIR:/reinforcement_learning" ubuntu:jammy /bin/bash -c "$DOCKER_CMD"
    elif command -v docker &> /dev/null; then
        docker run -it -v "$REPO_DIR:/reinforcement_learning" ubuntu:jammy /bin/bash -c "$DOCKER_CMD"
    else
