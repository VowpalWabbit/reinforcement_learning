...
struct object_factory
{
  ...
  int create(std::unique_ptr<I>& retval, const std::string& name, Args&&... args)
  {
    return create(retval, name, std::forward<Args>(args)..., nullptr, nullptr);
  }
  // Only api_status is default
  int create(std::unique_ptr<I>& retval, const std::string& name, Args&&... args, i_trace* trace)
  {
    return create(retval, name, std::forward<Args>(args)..., trace, nullptr);
  }
  // Only trace_logger is default
  int create(std::unique_ptr<I>& retval, const std::string& name, Args&&... args, api_status* status)
  {
    return create(retval, name, std::forward<Args>(args)..., nullptr, status);
  }

  int create(std::unique_ptr<I>& retval, const std::string& name, Args&&... args, i_trace* trace, api_status* status)
  {
