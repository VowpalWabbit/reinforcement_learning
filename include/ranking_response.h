/**
 * @brief ranking_response definition. ranking_response is returned from choose_rank call.  It contains the chosen
 * action and probability distribution.
 *
 ...
 * ranking_response contains all the actions and distribution from with the action was sampled.  It also contains the
 * chosen action id and the unique event_id representing the choice.  This unique event_id must be used to report back
 * outcomes against this choice for the online trainer.
 */
...
public:
   ...
   *
   * @param event_id The unique identifier for this interaction.  This event_id must be used when reporting the outcome
   * for this action
   */
   ...
   */
  int get_chosen_action_id(
      size_t& action_id, api_status* status = nullptr) const;  // id of the top action chosen by the model
  ...
  /**
   * @brief Set the chosen action id, but do not verify the index fits within the ranking.  (This is set internally by
   * the API) This is used in CCB where subsequent ranking_responses have subsets of the orignal actionset.
   *
   * @param action_id Chosen action id
   * @param status Optional field with detailed string description if there is an error
   * @return int Error code
