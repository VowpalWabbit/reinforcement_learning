...
public:
   ...
   */
  template <typename ErrCntxt>
  explicit live_model_episodic(const utility::configuration& config, error_fn_t<ErrCntxt> fn,
  ...
  live_model_episodic(const live_model_episodic&) =
...
private:
  ...
  bool _initialized = false;  //! Guard to ensure that live_model_episodic is properly initialized. i.e. init() was
 ...
 */
template <typename ErrCntxt>
live_model_episodic::live_model_episodic(const utility::configuration& config, error_fn_t<ErrCntxt> fn,
    ...
    : live_model_episodic(config, std::bind(fn, std::placeholders::_1, err_context), trace_factory, t_factory,
