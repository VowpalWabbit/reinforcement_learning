...
public:
  ...
  VW_DLL_PUBLIC explicit data_buffer(size_t body_size = 1024);

  // Get a pointer to beginning of preamble
  VW_DLL_PUBLIC value_type *preamble_begin();

  VW_DLL_PUBLIC size_t preamble_size() const;

  // Return pointer to beginning of buffer, starting at offset from the real beginning.
  VW_DLL_PUBLIC value_type *body_begin();

  // Body size (does not include the preamble)
  VW_DLL_PUBLIC size_t body_filled_size() const;

  // How large can we make the Body? (does not include the preamble.)
  VW_DLL_PUBLIC size_t body_capacity() const;

  // Offset for beginning of the body region from the beginning of the buffer
  VW_DLL_PUBLIC size_t get_body_beginoffset() const;

  // Offset for beginning of the body data from the beginning of the buffer
  VW_DLL_PUBLIC int set_body_beginoffset(size_t beginoffset);

  // Offset for end of the body data from the beginning of the buffer
  VW_DLL_PUBLIC size_t get_body_endoffset() const;

  // Offset for end of the body data from the beginning of the buffer
  VW_DLL_PUBLIC int set_body_endoffset(size_t endoffset);

  // Size of the entire filled buffer (body + preamble)
  VW_DLL_PUBLIC size_t buffer_filled_size() const;

  // Will resize entire buffer
  VW_DLL_PUBLIC void resize_body_region(size_t size);

  // Clear the contents of the buffer
  VW_DLL_PUBLIC void reset();

  // Get the beginning of the raw buffer
  VW_DLL_PUBLIC value_type *raw_begin();
