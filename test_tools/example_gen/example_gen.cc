...
void send_ccb_outcome(std::mt19937& rng, bool gen_random_reward, const char * event_id, r::live_model& rl, r::api_status& status) {
...
int take_action(r::live_model &rl, const char *event_id, int action,
  ...
  switch(action) {
    ...
    case CCB_BASELINE_ACTION_LOOP: { // "ccb action and random number of float rewards and mix of slot ids / non slot ids / float / string rewards"
      ...
      {
        r::multi_slot_response response;
        if(rl.request_multi_slot_decision(event_id, JSON_CCB_CONTEXT, action_flag, response, &baselines[0], 2, &status) != err::success)
            std::cout << status.get_error_msg() << std::endl;
      }
      else
      {
        r::multi_slot_response response;
        if(rl.request_multi_slot_decision(event_id, JSON_CCB_WITH_SLOT_ID_CONTEXT, action_flag, response, &baselines[0], 2, &status) != err::success)
            std::cout << status.get_error_msg() << std::endl;
      }
...
int run_config(int action, int count, int initial_seed, bool gen_random_reward,
  ...
  for(int i = 0; i < count; ++i) {
    char event_id[128];
    if(initial_seed == -1)
      strcpy_s(event_id, "abcdefghijklm");
    else
      sprintf_s(event_id, "%x", pseudo_random(initial_seed + i * 997739));
...
int main(int argc, char *argv[]) {
  ...
  desc.add_options()
    ...
    ("kind", po::value<std::string>(), "which kind of example to generate (cb,invalid-cb,ccb,ccb-with-slot-id,ccb-baseline,slates,ca,cb-loop,ca-loop,ccb-loop,ccb-baseline-loop,slates-loop,(f|s)(s|i|mix|i-out-of-bound)?-reward,action-taken)")
  ...
  try {
    ...
    std::vector<std::string> deferrable_interactions {
      ...
      "ca-loop", "ccb-with-slot-id", "ccb-loop", "ccb-baseline-loop", "slates-loop"
    };
