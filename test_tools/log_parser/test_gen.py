...
class Env:
    ...
    def cb_pdrop_05(self):
        ...
        with BinLogWriter(
            self.valid_joined_logs.joinpath("cb_joined_with_pdrop_05.fb")
        ) as with_pdrop:
            ...
            with_pdrop.write_regular_message(
                [
                    data.JoinedEvent(
                        event=data.CbEvent(
                            id="id", context=ctx1, actions=[2, 1], pass_prob=0.5
                        )
                    ),
                    data.JoinedEvent(event=data.OutcomeEvent(primary_id="id", value=1)),
                ]
            )
    ...
    def cb_pdrop_1(self):
        ...
        with BinLogWriter(
            self.valid_joined_logs.joinpath("cb_joined_with_pdrop_1.fb")
        ) as with_pdrop:
            with_pdrop.write_file_magic()
            with_pdrop.write_checkpoint_info()
            with_pdrop.write_regular_message(
                [
                    data.JoinedEvent(
                        event=data.CbEvent(
                            id="id1", context=ctx1, actions=[2, 1], pass_prob=0
                        )
                    ),
                    data.JoinedEvent(
            ...
            with_pdrop.write_regular_message(
                [
                    data.JoinedEvent(
                        event=data.CbEvent(
                            id="id2", context=ctx2, actions=[2, 1], pass_prob=1
                        )
                    ),
                    data.JoinedEvent(
    ...
    def multistep_2_episodes(self):
        ...
        with BinLogWriter(
            self.valid_joined_logs.joinpath("multistep_2_episodes.fb")
        ) as writer:
            ...
            writer.write_regular_message(
                [
                    data.JoinedEvent(
                        event=data.MultiStepEvent(
                            episode_id="ep1", event_id="1", context=ctx.format("A")
                        )
                    ),
                    data.JoinedEvent(
                        event=data.MultiStepEvent(
                            ...
                            event_id="2",
                            previous_id="1",
                            context=ctx.format("B"),
                        )
                    ),
                    data.JoinedEvent(
                        event=data.OutcomeEvent(
                            primary_id="ep1", secondary_id="1", value=2
                        )
                    ),
                ]
            )
            writer.write_regular_message(
                [
                    data.JoinedEvent(
                        event=data.MultiStepEvent(
                            episode_id="ep2", event_id="1", context=ctx.format("C")
                        )
                    ),
                    data.JoinedEvent(
                        event=data.MultiStepEvent(
    ...
    def multistep_3_deferred_episodes(self):
        ...
        with BinLogWriter(
            self.valid_joined_logs.joinpath("multistep_3_deferred_episodes.fb")
        ) as writer:
            writer.write_file_magic()
            writer.write_checkpoint_info()
            writer.write_regular_message(
            ...
            writer.write_regular_message(
                [
                    ...
                    data.JoinedEvent(event=data.OutcomeEvent(primary_id="ep2")),
                ]
            )
            writer.write_regular_message(
                [
    ...
    def multistep_unordered_episodes(self):
        ...
        with BinLogWriter(
            self.valid_joined_logs.joinpath("multistep_unordered_episodes.fb")
        ) as writer:
            writer.write_file_magic()
            writer.write_checkpoint_info()
            # 1(A) -> 2(B)
            ...
            msg = [
                ...
                data.JoinedEvent(
                    event=data.MultiStepEvent(
                        episode_id="ep2", event_id="4", context=ctx.format("F")
                    ),
                    timestamp=dt_after,
                ),
                data.JoinedEvent(
