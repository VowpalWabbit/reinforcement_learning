...
def parse_cb(payload, verbose):
    ...
    if not verbose:
        print(
            f"\tcb: actions:{evt.ActionIdsLength()} model:{evt.ModelId()} lm:{learning_mode_name(evt.LearningMode())} deferred:{evt.DeferredAction()}"
        )
    else:
        print(
            f"\tcb: actions:{evt.ActionIdsAsNumpy()} probs: {evt.ProbabilitiesAsNumpy()} model:{evt.ModelId()} lm:{learning_mode_name(evt.LearningMode())} deferred:{evt.DeferredAction()}"
        )
...
def parse_multislot(payload):
    ...
    print(
        f"\tmulti-slot slots:{evt.SlotsLength()} model:{evt.ModelId()} deferred:{evt.DeferredAction()} has-baseline:{not evt.BaselineActionsIsNone()}"
    )
    print(f"\t\tcontext: {fmt_payload(evt.ContextAsNumpy())}")
    if not evt.BaselineActionsIsNone():
        print(
            f'\t\tbaselines: {" ".join([str(b) for b in evt.BaselineActionsAsNumpy()])}'
        )
...
def parse_multistep(payload):
    ...
    print(
        f"\tmultistep: index: {evt.EventId()}\t actions:{evt.ActionIdsLength()} model:{evt.ModelId()}"
    )
    print(f"\t\tcontext: {fmt_payload(evt.ContextAsNumpy())}")
...
def parse_continuous_action(payload):
    ...
    print(
        f"\tcontinuous-action: action:{evt.Action()} pdf-value:{evt.PdfValue()} deferred:{evt.DeferredAction()}"
    )
    print(f"\t\tcontext: {fmt_payload(evt.ContextAsNumpy())}")
    ...
    for msg in reader.messages():
        if msg[0] == MSG_TYPE_REGULAR:
            ...
            for i in range(msg.EventsLength()):
                ...
                dump_event(
                    joined_event.EventAsNumpy(), i, joined_event.Timestamp(), verbose
                )
        elif msg[0] == MSG_TYPE_CHECKPOINT:
        ...
        if msg[0] == MSG_TYPE_REGULAR:
            ...
            yield {
                ...
                "message": {
                    header.Properties(i)
                    .Key()
                    .decode("utf-8"): header.Properties(i)
                    .Value()
                    .decode("utf-8")
                    for i in range(header.PropertiesLength())
                },
...
def is_binary_log_msg(buf):
    ...
    return (
        msg_id == MSG_TYPE_FILEMAGIC
        or msg_id == MSG_TYPE_HEADER
        or msg_id == MSG_TYPE_CHECKPOINT
        or msg_id == MSG_TYPE_REGULAR
        or msg_id == MSG_TYPE_EOF
    )
...
def main():
    ...
    parser.add_argument(
        "files", metavar="N", type=str, nargs="+", help="files to parse"
    )
    parser.add_argument(
