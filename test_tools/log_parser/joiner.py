...
arg_parser.add_argument(
    "--output", type=str, help="output file (defaults to merged.log)"
)
arg_parser.add_argument(
    "--verbose",
    type=bool,
...
arg_parser.add_argument(
    "-c",
    action="store_true",
    help="generate corrupt files for binary parser unit tests",
)
...
if args.c:
    ...
    bad_file_combinations = [
        (
            "corrupt_joined_payload.log",
            "cb_v2_size_2.fb",
            "f-reward_v2_size_2.fb",
            True,
            False,
            True,
            False,
        ...
        (
            "interaction_with_no_observation.log",
            "cb_v2_size_2.fb",
            "f-reward_v2_size_2.fb",
            False,
            False,
        ...
        (
            "no_interaction_but_with_observation.log",
            "cb_v2_size_2.fb",
            "f-reward_v2_size_2.fb",
            True,
            False,
    ...
    for (
        ...
        no,
        imt,
    ) in bad_file_combinations:
        print("---------")
        print(
            f"generating {f_name} using interactions file {interaction_file_name}, observations file {observations_file_name}"
        )
        print("---------")
        interactions_file = PreambleStreamReader(interaction_file_name)
        observations_file = PreambleStreamReader(observations_file_name)
        ...
        for msg in interactions_file.messages():
            ...
            for i in range(0, batch.EventsLength()):
                ...
                if multiple_batches:
                    print(f"batch with {len(events_to_serialize)} events")
                    print(f"joining iters with {batch.Metadata().ContentEncoding()}")
                    bin_bad_payload.write_join_msg(
                        events_to_serialize,
                        mess_with_payload=mess_with_payload_while_writing,
                        empty_payload=empty_payload_while_writing,
                        one_invalid_msg_type=one_invalid_msg_type_while_writing,
                    )
                    mess_with_payload_while_writing = (
                        False  # only mess with the first payload
                    )
                    empty_payload_while_writing = (
                        False  # only mess with the first payload
                    )
                    one_invalid_msg_type_while_writing = (
                        False  # only mess with the first payload
                    )
