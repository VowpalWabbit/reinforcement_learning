...
def create_readable_model(model_file, output_dir, vw_bin, empty_file):
    ...
    check_result(
        subprocess.run(
            cmd_str.split(), stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
    )
...
def build_combined_log(
    combined_log_file, log_files, start_after_event_id, final_event_id
):
    count = 0
    stop = False
            ...
            for x in open(log_file, "rb"):
                if found_first_event:
                    ...
                    if b'"' + final_event_id + b'"' in x:
                        print(
                            "Found end event id at line: {} in {}. Stop!".format(
                                i, log_file
                            )
                        )
                        stop = True
                    ...
                    if b'"' + start_after_event_id + b'"' in x:
                        ...
                        print(
                            "Found initial event id at line: {} in {}. Start!".format(
                                i, log_file
                            )
                        )
                i += 1
...
def run_test_model_reproducibility(
    ...
    final_readable_model_file = create_readable_model(
        final_model, output_dir, vw_bins[0], empty_file_name
    )

    # Extract event ids used from model files
    initial_model_first_id, initial_model_last_id = get_model_id(initial_model).split(
        b"/"
    )
    ...
    if not os.path.isfile(combined_log_file):
        build_combined_log(
            combined_log_file, log_files, initial_model_last_id, final_model_last_id
        )

    results = []
    results.append(
        (get_file_hash(final_readable_model_file), final_readable_model_file)
    )
    ...
    for v in vw_bins:
        ...
        reproduced_readable_file = create_readable_model(
            reproduced_model_file, output_dir, vw_bins[0], empty_file_name
        )
        results.append(
            (get_file_hash(reproduced_readable_file), reproduced_readable_file)
        )
        i += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--final_model", help="Final model that should be reproduced", required=True
    )
    parser.add_argument(
        "--log_files", nargs="+", help="dsjson logs to replay", required=True
    )
    parser.add_argument(
        "--vw_bins", nargs="+", help="vw_binaries to test against", required=True
    )
    parser.add_argument(
        "--output_dir", help="Directory to output files in", default=os.getcwd()
    )
    ...
    run_test_model_reproducibility(
        initial_model, final_model, log_files, vw_bins, output_dir
    )
