        ...
        while True:
            ...
            for i in range(0, batch.EventsLength()):
                ...
                if m.PayloadType() == PayloadType.CB:
                    ...
                    m.PayloadType() == PayloadType.CCB
                    or m.PayloadType() == PayloadType.Slates
                ):
                    yield (
                        m.PayloadType(),
                        MultiSlotEvent.GetRootAsMultiSlotEvent(payload, 0),
                    )
                elif m.PayloadType() == PayloadType.Outcome:
                    yield (
                        m.PayloadType(),
                        OutcomeEvent.GetRootAsOutcomeEvent(payload, 0),
                    )
                    ...
                    yield (
                        m.PayloadType(),
                        MultiStepEvent.GetRootAsMultiStepEvent(payload, 0),
                    )
                elif m.PayloadType() == PayloadType.Episode:
                    yield (
                        m.PayloadType(),
                        EpisodeEvent.GetRootAsEpisodeEvent(payload, 0),
                    )
                else:
...
def compare_outcome(base, compare):
    if (
        base.ValueType() != compare.ValueType()
        or base.IndexType() != compare.IndexType()
    ):
...
def main():
    ...
    parser.add_argument("--base_dir", type=str, help="Base files location")
    parser.add_argument("--compare_dir", help="Comparison files location")
    args = parser.parse_args()

    base_files = {
        f
        for f in os.listdir(args.base_dir)
        if os.path.isfile(os.path.join(args.base_dir, f))
    }
    compare_files = {
        f
        for f in os.listdir(args.compare_dir)
        if os.path.isfile(os.path.join(args.compare_dir, f))
    }

    if base_files != compare_files:
        raise Exception(
            "base_dir and compare_dir must contain the same number of files with the same names"
        )
    ...
    for filename in base_files:
        ...
        for base, compare in zip(base_reader.messages(), compare_reader.messages()):
            if base is None or compare is None:
                raise Exception(
                    "base and compare files do not contain the same number of events"
                )
            if base[0] != compare[0]:
